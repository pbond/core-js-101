...
function findFirstSingleChar(str) {
  const char = str.split('')
    .find((c) => str.indexOf(c) === str.lastIndexOf(c));
...
function getDigitalRoot(num) {
  return Number(num)
    ...
    .split('')
    .reduce((acc, val) => {
      let sum = acc + parseInt(val, 10);
      if (sum > 9) sum = 1 + (sum % 10);
      return sum;
    }, 0);
}
  ...
  try {
    const sequence = str.split('')
      .reduce((acc, val) => {
        ...
        throw new Error('Brackets is not balanced');
      }, []);
...
function getCommonDirectoryPath(pathes) {
  ...
  const common = path.reduce((acc, val) => {
    if (val.length > 0 && pathes.every((p) => p.indexOf(`${val}/`) === acc.length)) {
      return `${acc + val}/`;
    }
    return acc;
  }, '/');
  return pathes.every((p) => p.indexOf(common) === 0) ? common : '';
}
...
function evaluateTicTacToePosition(position) {
  const isLine = position.reduce((acc, val) => {
  ...
  const isColumn = position.reduce((acc, val, index) => {
    if (acc === 'X' || acc === '0') return acc;
